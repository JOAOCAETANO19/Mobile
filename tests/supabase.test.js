...
test('config: salva, normaliza barra, valida domínio', () => {
  const s = memStorage();
  clearConfig(s);
  assert.deepEqual(loadConfig(s), DEFAULT_CONFIG, 'sem config manual → usa o projeto embutido');
  ...
  assert.equal(c.key, 'abc'); // config manual tem prioridade sobre a embutida
});
